...
// **Static IP Configuration for ESP5**
IPAddress staticIP(192,168,4,7);  
IPAddress gateway(192,168,4,1);
IPAddress subnet(255,255,255,0);

// **ESP5 LED Configuration**
#define LED_PIN3 18  
#define LED_PIN4 25  
#define POT_PIN 34  
...
void sendDataToLaptop(int potValue) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (laptop.connect(LAPTOP_IP, 80)) {  
            ...
            laptop.stop();
            Serial.println("[ESP5] Sent Potentiometer Value to Laptop.");
        } else {
            Serial.println("[ESP5] Failed to connect to Laptop.");
        }
    }
}
...
void requestBrightness() {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (clientReceive.connect(LAPTOP_IP, 80)) {
            clientReceive.println("{\"esp_id\": 5, \"request\": \"brightness\"}");
            ...
            if (clientReceive.available()) {
                ...
                if (!error) {
                    ...
                    Serial.print("[ESP5] Received Brightness: ");
                    Serial.print(brightness3);
                    Serial.print(", ");
                    Serial.println(brightness4);
                } else {
                    Serial.println("[ESP5] Failed to parse brightness data.");
                }
            } else {
                Serial.println("[ESP5] No brightness data received.");
            }
            clientReceive.stop();
        } else {
            Serial.println("[ESP5] Failed to connect to laptop for brightness request.");
        }
    }
}
...
void setup() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        Serial.println("\n[ESP5] Connected to Wi-Fi. IP: " + WiFi.localIP().toString());
    } else {
...
void loop() {
    ...
    Serial.println("[ESP5] Loop Running...");

    // **ESP5's Potentiometer Data**
    int potValue = analogRead(POT_PIN);
    Serial.print("[ESP5] Potentiometer Value: ");
    ...
    if (abs(potValue - lastPotValue) > 60) {  
        ...
        Serial.println("[ESP5] Sending Pot Value to Laptop...");
    ...
    if (currentMillis - lastReceiveCheck > 100) {
        ...
        Serial.println("[ESP5] Requesting Brightness...");
        requestBrightness();
    }

    // **Update LED Brightness**
    Serial.println("[ESP5] Updating LEDs...");
    updateLEDs();

    // **Fade Out LEDs After 10 Seconds**
    if (currentMillis - lastPotChange > 10000 && !isFading) {
        Serial.println("[ESP5] Fading Out LEDs...");
